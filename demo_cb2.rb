...
def test_drive(midi_object,root_note,scale,chord,duration = 1)
  ...
  last_root_note = last_chord.root_note
  
  #hacky
  last_chord = last_chord.invert
  while last_chord.notes.first.name != last_root_note.name
    last_chord = last_chord.invert
  end
  ...
  #FIXME: not sure why the last_note seems to sustain for a longer time
end
